...
- `TAC_EVALUATE_MAX_CONCURRENCY`：阶段内 AI 评估并发，默认 `20`。
...
评估请求会把文章 Markdown 正文截断到最多 `50000` 个字符后再发送给 AI。

AI 评估输出使用严格 JSON schema。当前顶层字段为 `decision`、`content_type`、`summary`、`tags`、`suggested_tags` 和 `recommendation_reason`；不再包含独立 `confidence`、`dimensions` 或 `full_reasoning` 字段。`content_type` 用于区分技术文章、工程实践、科研议题、科研思考、学习路线、个人心得、职业经验和工具笔记。提示词仍会要求模型参考领域相关性、长期价值、内容深度、原创性、可迁移性和可读性做内部判断。系统只根据 `decision` 判断结果：`accept` 自动收录，`reject` 拒收，`low_confidence` 留待复核。
