...
Technical Article Curation is an AI-assisted pipeline for discovering, evaluating, and publishing high-quality technical articles. The project focuses on long-term engineering value rather than short-lived news or marketing content.

The MVP is data-first: it produces stable JSON and Markdown artifacts under `public/`, which can later be consumed by a website, internal knowledge base, or other publishing channel.
...
- Evaluate article quality with a strict JSON schema and Pydantic validation.
...
Accepted articles should show durable engineering value. Strong candidates usually include:
...
The default source configuration is in `config/sources.yaml`. MVP sources are verified RSS/Atom feeds from engineering teams, communities, and personal technical blogs.
...
The source configuration supports RSS/Atom feeds and manual URLs. Manual URLs should point to article pages, not general site homepages.
...
- `OPENAI_API_KEY`: API key for OpenAI-compatible evaluation.
...
- `TAC_MAX_RETRY`: maximum retry count for failed articles, default `3`.
- `TAC_CRAWLER4AI_ENABLED`: defaults to enabled. Set to `false`, `0`, `no`, `off`, or `disabled` to bypass Crawler4AI and use the fallback fetcher directly.
- `TAC_AI_RESPONSE_PATH`: test-only path to a fixed AI JSON response.
- `TAC_FETCH_FIXTURE_PATH`: test-only path to fixed Markdown content.
- `TAC_PROMPT_PATH`: prompt path, default `prompts/evaluate.md`.
- `TAC_FEW_SHOT_DIR`: few-shot directory, default `prompts/few_shots`.

## Fetching

The fetch stage defaults to Crawler4AI when the optional `crawler` extra is installed. If Crawler4AI is disabled or fails, the system falls back to:

- `requests`
- `BeautifulSoup`
- `markdownify`

The fallback fetcher uses browser-like headers, follows redirects, retries a small number of times, removes common non-content elements, and converts the main page content to Markdown.
...
The AI response must validate against the strict schema:
...
- `evaluations`: AI decision, confidence, dimensions, summary, tags, recommendation reason, full reasoning, model, and raw JSON.
...
Public JSON fields are limited to:
...
The test suite uses unit tests and a small offline end-to-end fixture. It avoids real network and real AI calls by using fixed RSS, Markdown, and AI JSON response files.
